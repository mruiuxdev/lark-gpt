...
async function handleReply(userInput, sessionId, messageId) {
  ...
  if (content.image) {
    ...
    await reply(messageId, imageResult.description);
  } else {
    // Handle text query
    const openaiResponse = await query({ question: text });
    await reply(messageId, openaiResponse.text);
  }
